          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => _AddEmployeeScreenFromDetail(employee: employee),
...
/// Internal screen used by EmployeeDetailScreen for editing employee data.
/// Delegates to the AddEmployeeScreen in employees_screen.dart.
class _AddEmployeeScreenFromDetail extends StatefulWidget {
  final Map<String, dynamic> employee;
  const _AddEmployeeScreenFromDetail({required this.employee});

  @override
  State<_AddEmployeeScreenFromDetail> createState() => _AddEmployeeScreenFromDetailState();
}

class _AddEmployeeScreenFromDetailState extends State<_AddEmployeeScreenFromDetail> {
  ...
  Future<void> _save() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تم تعديل الموظف بنجاح'), backgroundColor: AppColors.success),
      );
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                onChanged: null, // Read-only when editing
              ),
              const SizedBox(height: 14),
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                              ...
                              Expanded(
                                child: GestureDetector(
                                  ...
                                  child: Container(
                                    ...
                                    child: Text('له', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w700, color: _balanceType == 'credit' ? AppColors.success : AppColors.textHint)),
                                  ),
                                ),
                              ),
                              Expanded(
                                child: GestureDetector(
                                  ...
                                  child: Container(
                                    ...
                                    child: Text('عليه', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w700, color: _balanceType == 'debit' ? AppColors.error : AppColors.textHint)),
                                  ),
                                ),
                              ),
                            ],
