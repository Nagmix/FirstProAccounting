  ...
  Future<void> _confirmVoucher(int id) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('سيتم تعديل المخزون وإنشاء قيود محاسبية. هل أنت متأكد؟'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('إلغاء')),
          TextButton(
            onPressed: () => Navigator.pop(ctx, true),
            ...
            : _vouchers.isEmpty
                ? Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.fact_check, size: 64, color: AppColors.textHint),
                        const SizedBox(height: 16),
                        Text('لا توجد سندات جرد', style: theme.textTheme.titleMedium?.copyWith(color: AppColors.textSecondary)),
                      ],
                    ),
                  )
                : RefreshIndicator(
                                ...
                                children: [
                                  ...
                                  if (isDraft) ...[
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        TextButton.icon(
                                          onPressed: () => _confirmVoucher(v['id'] as int),
                                        ...
                                        TextButton.icon(
                                          onPressed: () => _deleteVoucher(v['id'] as int),
  ...
  Future<void> _showCreateVoucherDialog() async {
    ...
    for (final p in products) {
      qtyControllers[p['id'] as int] = TextEditingController(
        text: (p['current_stock'] as num?)?.toDouble().toStringAsFixed(0) ?? '0',
      );
    }
                  ...
                  child: Column(
                    children: [
                      ...
                      DropdownButtonFormField<int>(
                        ...
                        items: [
                          const DropdownMenuItem(value: null, child: Text('كل المستودعات')),
                            ...
                            return Padding(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  SizedBox(
                                    width: 60,
                                    child: Text('ن: ${systemQty.toStringAsFixed(0)}', style: Theme.of(context).textTheme.bodySmall),
                                  ),
                                  const SizedBox(width: 4),
                                  SizedBox(
                                    width: 70,
