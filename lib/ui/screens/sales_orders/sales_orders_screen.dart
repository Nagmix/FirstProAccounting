...
class _SalesOrdersScreenState extends State<SalesOrdersScreen>
  ...
  final List<MapEntry<String, String>> _statusTabs = [
    ...
    const MapEntry('confirmed', 'مؤكد'),
    const MapEntry('shipped', 'مشحون'),
  ...
  /// Convert a confirmed/shipped sales order to a sales invoice
  Future<void> _convertToInvoice(Map<String, dynamic> order) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تم تحويل طلب البيع إلى فاتورة مبيعات بنجاح'),
  ...
  void _showStatusMenu(String orderId, String currentStatus) {
    // Exclude 'converted' from manual status change options
    final changeableStatuses = Map.from(_statusLabels)..remove('converted');
  ...
  void _showCreateOrderDialog() {
    showModalBottomSheet(
      ...
      builder: (ctx) => _CreateSalesOrderForm(
        onSaved: () => _loadData(),
      ),
    );
  }
...
// ══════════════════════════════════════════════════════════════════
//  Create Sales Order Form (Bottom Sheet)
// ══════════════════════════════════════════════════════════════════

class _CreateSalesOrderForm extends StatefulWidget {
  final VoidCallback onSaved;
  const _CreateSalesOrderForm({required this.onSaved});

  @override
  State<_CreateSalesOrderForm> createState() => _CreateSalesOrderFormState();
}

class _CreateSalesOrderFormState extends State<_CreateSalesOrderForm> {
  ...
  Future<void> _saveOrder() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تم إنشاء طلب البيع $orderNumber بنجاح'),
            backgroundColor: AppColors.success,
          ),
        );
        widget.onSaved();
