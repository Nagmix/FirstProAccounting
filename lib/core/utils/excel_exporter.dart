...
class ExcelExporter {
  ...
  static Future<String> exportAccountsToExcel(
    ...
    for (var i = 0; i < accounts.length; i++) {
      ...
      sheet.cell(CellIndex.indexByString('A$row')).value =
          TextCellValue(account['account_code'] as String? ?? '');
      sheet.cell(CellIndex.indexByString('B$row')).value =
          TextCellValue(account['name_ar'] as String? ?? '');
      ...
      sheet.cell(CellIndex.indexByString('D$row')).value =
          TextCellValue(account['currency'] as String? ?? 'YER');
      sheet.cell(CellIndex.indexByString('E$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(account['balance']));
      sheet.cell(CellIndex.indexByString('F$row')).value = TextCellValue(
          (account['is_active'] as int?) == 1 ? 'نشط' : 'غير نشط');
    }
  ...
  /// تصدير الفواتير
  static Future<String> exportInvoicesToExcel(
    ...
    for (var i = 0; i < invoices.length; i++) {
      ...
      final row = i + 2;
      sheet.cell(CellIndex.indexByString('A$row')).value =
          TextCellValue(inv['id'] as String? ?? '');
      sheet.cell(CellIndex.indexByString('B$row')).value =
          TextCellValue(_invoiceTypeAr(inv['type'] as String? ?? ''));
      sheet.cell(CellIndex.indexByString('C$row')).value =
          TextCellValue(_formatDate(inv['created_at'] as String?));
      sheet.cell(CellIndex.indexByString('D$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(inv['subtotal']));
      sheet.cell(CellIndex.indexByString('E$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(inv['discount_amount']));
      sheet.cell(CellIndex.indexByString('F$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(inv['tax_amount']));
      sheet.cell(CellIndex.indexByString('G$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(inv['total']));
      sheet.cell(CellIndex.indexByString('H$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(inv['paid_amount']));
      sheet.cell(CellIndex.indexByString('I$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(inv['remaining']));
      sheet.cell(CellIndex.indexByString('J$row')).value =
          TextCellValue(inv['status'] as String? ?? '');
      sheet.cell(CellIndex.indexByString('K$row')).value =
          TextCellValue(inv['currency'] as String? ?? 'YER');
    }
  ...
  /// تصدير المخزون
  static Future<String> exportInventoryToExcel(
    ...
    for (var i = 0; i < products.length; i++) {
      ...
      final row = i + 2;
      sheet.cell(CellIndex.indexByString('A$row')).value =
          TextCellValue(p['item_code'] as String? ?? '');
      sheet.cell(CellIndex.indexByString('B$row')).value =
          TextCellValue(p['name_ar'] as String? ?? '');
      sheet.cell(CellIndex.indexByString('C$row')).value =
          TextCellValue(p['barcode'] as String? ?? '');
      sheet.cell(CellIndex.indexByString('D$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(p['cost_price']));
      sheet.cell(CellIndex.indexByString('E$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(p['sell_price']));
      sheet.cell(CellIndex.indexByString('F$row')).value =
          DoubleCellValue((p['current_stock'] as num?)?.toDouble() ?? 0.0);
      sheet.cell(CellIndex.indexByString('G$row')).value =
          DoubleCellValue((p['min_stock'] as num?)?.toDouble() ?? 0.0);
      sheet.cell(CellIndex.indexByString('H$row')).value =
          TextCellValue(p['warehouse_name'] as String? ?? '');
      sheet.cell(CellIndex.indexByString('I$row')).value =
          TextCellValue((p['is_active'] as int?) == 1 ? 'نشط' : 'غير نشط');
    }
  ...
  static Future<String> exportTransactionsToExcel(
    ...
    for (var i = 0; i < transactions.length; i++) {
      ...
      final row = i + 2;
      sheet.cell(CellIndex.indexByString('A$row')).value =
          TextCellValue(_formatDate(t['date'] as String?));
      sheet.cell(CellIndex.indexByString('B$row')).value = TextCellValue(
          t['account_name'] as String? ?? t['account_id']?.toString() ?? '');
      sheet.cell(CellIndex.indexByString('C$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(t['debit']));
      sheet.cell(CellIndex.indexByString('D$row')).value =
          DoubleCellValue(MoneyHelper.readMoney(t['credit']));
      sheet.cell(CellIndex.indexByString('E$row')).value =
  ...
  static Future<String> exportAccountStatementToExcel({
    ...
    for (var i = 0; i < sortedMovements.length; i++) {
      ...
      sheet.cell(CellIndex.indexByString('A$row')).value =
          TextCellValue(_formatDate(dateStr));
      sheet.cell(CellIndex.indexByString('B$row')).value =
          TextCellValue(description);
      sheet.cell(CellIndex.indexByString('C$row')).value =
          DoubleCellValue(debit);
      sheet.cell(CellIndex.indexByString('D$row')).value =
    ...
    sheet.cell(CellIndex.indexByString('B$totalRow')).value =
        TextCellValue('الإجمالي');
    sheet.cell(CellIndex.indexByString('C$totalRow')).value =
        DoubleCellValue(totalDebit);
    sheet.cell(CellIndex.indexByString('D$totalRow')).value =
        DoubleCellValue(totalCredit);
    sheet.cell(CellIndex.indexByString('E$totalRow')).value =
        DoubleCellValue(netBalance);
    // Bold the total row
    for (var col = 0; col < 5; col++) {
      final cell = sheet.cell(
          CellIndex.indexByColumnRow(columnIndex: col, rowIndex: totalRow - 1));
      cell.cellStyle = CellStyle(bold: true);
    }

    return await _saveAndShare(
        excel, 'كشف_حساب_${entityName.replaceAll(' ', '_')}');
  }
  ...
  static Future<String> exportGenericReport({
    ...
    final excel = Excel.createExcel();
    final sheetName =
        reportName.length > 31 ? reportName.substring(0, 31) : reportName;
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      for (var colIdx = 0; colIdx < columns.length; colIdx++) {
        final cell = sheet.cell(CellIndex.indexByColumnRow(
            columnIndex: colIdx, rowIndex: excelRow - 1));
        ...
        if (value == null) {
          ...
          if (columns[colIdx] == 'التاريخ' ||
              columns[colIdx].contains('تاريخ')) {
    ...
    if (totals.isNotEmpty) {
      ...
      for (var colIdx = 0; colIdx < columns.length; colIdx++) {
        final cell = sheet.cell(CellIndex.indexByColumnRow(
            columnIndex: colIdx, rowIndex: totalRowIdx - 1));
        ...
        cell.cellStyle = CellStyle(
            bold: true, backgroundColorHex: ExcelColor.fromHexString('E8EAF6'));
      }
    }
