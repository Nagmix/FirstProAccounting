...
class MigrationV2ToV10 {
  ...
  static Future<void> migrateV4(Database db) async {
      ...
      try { await db.execute('ALTER TABLE invoices ADD COLUMN payment_mechanism TEXT NOT NULL DEFAULT \'cash\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN payment_method TEXT NOT NULL DEFAULT \'cash\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN is_return INTEGER NOT NULL DEFAULT 0'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN cash_box_id INTEGER'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Add balance_type to customers
      try { await db.execute('ALTER TABLE customers ADD COLUMN balance_type TEXT NOT NULL DEFAULT \'credit\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Add balance_type to suppliers (default 'credit' because we typically owe the supplier)
      try { await db.execute('ALTER TABLE suppliers ADD COLUMN balance_type TEXT NOT NULL DEFAULT \'credit\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Add linked_cash_box_id to accounts
      try { await db.execute('ALTER TABLE accounts ADD COLUMN linked_cash_box_id INTEGER'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Change currency default in accounts
      try { await db.execute('UPDATE accounts SET currency = \'YER\' WHERE currency = \'SAR\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('UPDATE suppliers SET currency = \'YER\' WHERE currency = \'SAR\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      ...
      try { await db.execute('UPDATE products SET tax_rate = 0.0 WHERE tax_rate = 15.0'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
  ...
  static Future<void> migrateV5(Database db) async {
      ...
      try { await db.execute('ALTER TABLE invoices ADD COLUMN currency TEXT NOT NULL DEFAULT \'YER\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN exchange_rate REAL NOT NULL DEFAULT 1.0'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Add currency column to customers
      try { await db.execute('ALTER TABLE customers ADD COLUMN currency TEXT NOT NULL DEFAULT \'YER\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
  }
  ...
  static Future<void> migrateV6(Database db) async {
      ...
      try { await db.execute('ALTER TABLE invoices ADD COLUMN transport_charges REAL NOT NULL DEFAULT 0.0'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Delete AED and KWD currencies
      try { await db.delete('currencies', where: 'code IN (?, ?)', whereArgs: ['AED', 'KWD']); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
  ...
  static Future<void> migrateV7(Database db) async {
      ...
      try { await db.execute('ALTER TABLE invoices ADD COLUMN ewallet_provider TEXT'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN bank_transfer_provider TEXT'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN transfer_number TEXT'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN attachment_path TEXT'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
  }
  ...
  static Future<void> migrateV8(Database db) async {
      ...
      try { await db.execute('ALTER TABLE accounts ADD COLUMN debt_ceiling REAL NOT NULL DEFAULT 0.0'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE accounts ADD COLUMN balance_type TEXT NOT NULL DEFAULT \'credit\''); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Add attachment_path, operation_type, expense_account_id to expenses
      try { await db.execute('ALTER TABLE expenses ADD COLUMN attachment_path TEXT'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      ...
      try { await db.execute('CREATE INDEX IF NOT EXISTS idx_expenses_expense_account_id ON expenses (expense_account_id)'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
  }
