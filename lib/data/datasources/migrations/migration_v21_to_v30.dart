...
class MigrationV21ToV30 {
  ...
  static Future<void> migrateV21(Database db) async {
    ...
    try {
      await db.execute(
          "ALTER TABLE customers ADD COLUMN contact_method TEXT DEFAULT 'whatsapp'");
    } catch (e) {
    ...
    try {
      await db.execute(
          'ALTER TABLE customers ADD COLUMN debt_ceiling REAL NOT NULL DEFAULT 0.0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    ...
    try {
      await db.execute(
          "UPDATE customers SET contact_method = COALESCE(notification_method, 'whatsapp') WHERE contact_method IS NULL OR contact_method = 'whatsapp'");
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db.execute(
          'UPDATE customers SET debt_ceiling = COALESCE(credit_limit, 0.0) WHERE debt_ceiling = 0.0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
  }
  ...
  static Future<void> migrateV22(Database db) async {
    ...
    // v22 indexes
    await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_inventory_vouchers_number ON inventory_vouchers (voucher_number)');
    await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_inventory_vouchers_date ON inventory_vouchers (date)');
    await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_inventory_vouchers_warehouse ON inventory_vouchers (warehouse_id)');
    await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_inventory_vouchers_status ON inventory_vouchers (status)');
    await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_inventory_voucher_items_voucher ON inventory_voucher_items (voucher_id)');
    await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_inventory_voucher_items_product ON inventory_voucher_items (product_id)');
    await db.execute(
  ...
  static Future<void> migrateV24(Database db) async {
    ...
    for (final row in affectedAccounts) {
      ...
      final correctBalance =
          MoneyHelper.readCalculatedMoney(txResult.first['net_debit']);
      await db.update(
        'accounts',
        {
    ...
    try {
      await db.execute(
          'ALTER TABLE products ADD COLUMN average_cost REAL NOT NULL DEFAULT 0.0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    // Initialize average_cost from existing cost_price
    await db.execute(
        'UPDATE products SET average_cost = cost_price WHERE average_cost = 0.0 AND cost_price > 0.0');
  ...
  static Future<void> migrateV25(Database db) async {
    ...
    try {
      await db.execute('ALTER TABLE products ADD COLUMN base_unit_id INTEGER');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db
          .execute('ALTER TABLE products ADD COLUMN purchase_unit_id INTEGER');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db.execute('ALTER TABLE products ADD COLUMN sale_unit_id INTEGER');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db.execute(
          'ALTER TABLE products ADD COLUMN tax_inclusive INTEGER NOT NULL DEFAULT 0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
    ...
    try {
      await db.execute(
          'ALTER TABLE products ADD COLUMN is_purchasable INTEGER NOT NULL DEFAULT 1');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db.execute(
          'ALTER TABLE products ADD COLUMN allow_negative INTEGER NOT NULL DEFAULT 0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db.execute(
          'ALTER TABLE products ADD COLUMN sell_retail INTEGER NOT NULL DEFAULT 1');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
    ...
    try {
      await db.execute('ALTER TABLE products ADD COLUMN supplier_code TEXT');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    ...
    await db.execute(
        'UPDATE products SET base_unit_id = unit_id WHERE base_unit_id IS NULL AND unit_id IS NOT NULL');
    await db.execute(
        'UPDATE products SET sale_unit_id = unit_id WHERE sale_unit_id IS NULL AND unit_id IS NOT NULL');
    await db.execute(
        'UPDATE products SET purchase_unit_id = unit_id WHERE purchase_unit_id IS NULL AND unit_id IS NOT NULL');

    // ── Add unit fields to invoice_items ──
    try {
      await db.execute('ALTER TABLE invoice_items ADD COLUMN unit_name TEXT');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db.execute(
          'ALTER TABLE invoice_items ADD COLUMN conversion_factor REAL NOT NULL DEFAULT 1.0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db.execute(
          'ALTER TABLE invoice_items ADD COLUMN base_quantity REAL NOT NULL DEFAULT 1.0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    ...
    await db.execute(
        'UPDATE invoice_items SET base_quantity = quantity WHERE base_quantity = 1.0 AND quantity != 1.0');

    // ── Update unit_conversions to use unit IDs ──
    try {
      await db.execute(
          'ALTER TABLE unit_conversions ADD COLUMN from_unit_id INTEGER');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db.execute(
          'ALTER TABLE unit_conversions ADD COLUMN to_unit_id INTEGER');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
  }
  ...
  static Future<void> migrateV26(Database db) async {
    ...
    try {
      await db.execute(
          'ALTER TABLE products ADD COLUMN expiry_tracking INTEGER NOT NULL DEFAULT 0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
  ...
  static Future<void> migrateV27(Database db) async {
    try {
      await db
          .execute('ALTER TABLE products ADD COLUMN cogs_account_id INTEGER');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
    try {
      await db
          .execute('ALTER TABLE products ADD COLUMN vat_account_id INTEGER');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
  }
  ...
  static Future<void> migrateV30(Database db) async {
    try {
      await db.execute(
          'ALTER TABLE stocktaking_items ADD COLUMN variance REAL NOT NULL DEFAULT 0.0');
    } catch (e) {
      MigrationHelpers.logMigrationError("migration", e);
    }
  }
