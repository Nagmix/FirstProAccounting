...
class MigrationV11ToV20 {
  ...
  static Future<void> migrateV12(Database db) async {
      ...
      try { await db.execute('ALTER TABLE invoices ADD COLUMN shift_id INTEGER'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN cashier_name TEXT'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE invoices ADD COLUMN is_posted INTEGER NOT NULL DEFAULT 0'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Create indexes for new invoice columns
      try { await db.execute('CREATE INDEX IF NOT EXISTS idx_invoices_shift_id ON invoices (shift_id)'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('CREATE INDEX IF NOT EXISTS idx_invoices_is_posted ON invoices (is_posted)'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
  ...
  static Future<void> migrateV13(Database db) async {
      ...
      try { await db.execute("ALTER TABLE cash_boxes ADD COLUMN currency TEXT NOT NULL DEFAULT 'YER'"); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Add cashier_name column to shifts
      try { await db.execute("ALTER TABLE shifts ADD COLUMN cashier_name TEXT"); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
  }
  ...
  static Future<void> migrateV20(Database db) async {
      ...
      try { await db.execute("UPDATE suppliers SET balance_type = 'credit' WHERE balance_type = 'debit' AND balance >= 0"); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }

      // Add customer_id and supplier_id columns to vouchers
      try { await db.execute('ALTER TABLE vouchers ADD COLUMN customer_id INTEGER'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
      try { await db.execute('ALTER TABLE vouchers ADD COLUMN supplier_id INTEGER'); } catch (e) { MigrationHelpers.logMigrationError("migration", e); }
