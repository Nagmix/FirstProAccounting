...
class ExpenseRepository {
  ...
  Future<void> saveExpenseWithJournalEntry(Map<String, dynamic> expenseMap) async {
    ...
    final amountBase = MoneyHelper.readMoney(expenseMap['amount_base']) > 0
        ...
        : expectedBase;
    if (MoneyHelper.readMoney(expenseMap['amount_base']) <= 0 && expectedBase > 0) {
    ...
    final bool needsYerConversion = expenseCurrency != 'YER' && exchangeRate > 0;
    final int codeOffset = needsYerConversion ? 0 : (expenseCurrency == 'SAR' ? 1 : (expenseCurrency == 'USD' ? 2 : 0));
    ...
    await db.transaction((txn) async {
      ...
      if (needsYerConversion) {
        ...
        final expenseAccount = await txn.query('accounts', where: 'account_code = ? AND currency = ?', whereArgs: ['5000', 'YER'], limit: 1);
      ...
      if (expenseAccId == null) {
        throw Exception('لا يوجد حساب مصروفات للعملة ${needsYerConversion ? "YER" : expenseCurrency}. يرجى إنشاء حساب مصروفات أولاً.');
      }
      ...
      if (cashAccountId == null) {
        throw Exception('لا يوجد حساب نقدية/بنك للعملة $journalCurrency. يرجى إنشاء حساب صناديق وبنوك أولاً.');
      }
      ...
      if (cashBoxId != null && amount > 0) {
        ...
        final cashBoxAmountCents = MoneyHelper.toCents(amount); // native currency
        if (isSarf) {
          // صرف: النقدية تخرج من الصندوق
          if (cashBalanceType == 'credit') {
            await txn.rawUpdate('UPDATE cash_boxes SET balance = balance - ?, updated_at = ? WHERE id = ?', [cashBoxAmountCents, now, cashBoxId]);
          } else {
            await txn.rawUpdate('UPDATE cash_boxes SET balance = balance + ?, updated_at = ? WHERE id = ?', [cashBoxAmountCents, now, cashBoxId]);
          }
        } else {
    ...
    await db.transaction((txn) async {
      ...
      if (oldAmountBase > 0 && oldExpenseAccountId != null) {
        ...
        if (oldCashBoxId != null && oldAmountBase > 0) {
          ...
          final oldCashBoxAmountCents = MoneyHelper.toCents(oldAmount > 0 ? oldAmount : oldAmountBase);
          if (oldIsSarf) {
            // Original was صرف (cash out): reverse = cash in
            if (oldCashBalanceType == 'credit') {
              await txn.rawUpdate('UPDATE cash_boxes SET balance = balance + ?, updated_at = ? WHERE id = ?', [oldCashBoxAmountCents, now, oldCashBoxId]);
            } else {
              await txn.rawUpdate('UPDATE cash_boxes SET balance = balance - ?, updated_at = ? WHERE id = ?', [oldCashBoxAmountCents, now, oldCashBoxId]);
            }
          } else {
      ...
      final newExchangeRate = (newExpenseMap['exchange_rate'] as num?)?.toDouble() ?? 1.0;
      final bool needsYerConversion = newCurrency != 'YER' && newExchangeRate > 0;
      final int codeOffset = needsYerConversion ? 0 : (newCurrency == 'SAR' ? 1 : (newCurrency == 'USD' ? 2 : 0));
      final String journalCurrency = needsYerConversion ? 'YER' : newCurrency;
      final double journalAmount = needsYerConversion ? newAmountBase : newAmount;
      ...
      if (newExpenseAccountId != null && newAmountBase > 0) {
        ...
        if (needsYerConversion) {
          final expenseAccount = await txn.query('accounts', where: 'account_code = ? AND currency = ?', whereArgs: ['5000', 'YER'], limit: 1);
          effectiveExpenseAccountId = expenseAccount.isNotEmpty ? expenseAccount.first['id'] as int : null;
        ...
        if (cashBoxId != null && newAmount > 0) {
          final cashBoxRow = await txn.query('cash_boxes', where: 'id = ?', whereArgs: [cashBoxId], limit: 1);
          final cashBalanceType = cashBoxRow.isNotEmpty ? (cashBoxRow.first['balance_type'] as String? ?? 'credit') : 'credit';
          final cashBoxAmountCents = MoneyHelper.toCents(newAmount); // native currency
          if (isSarf) {
            // صرف: النقدية تخرج
            if (cashBalanceType == 'credit') {
              await txn.rawUpdate('UPDATE cash_boxes SET balance = balance - ?, updated_at = ? WHERE id = ?', [cashBoxAmountCents, now, cashBoxId]);
  ...
  Future<List<Map<String, dynamic>>> getExpensesBySubAccountId(int subAccountId, {String orderBy = 'expense_date DESC'}) async {
    final db = await _db;
    return await db.query('expenses', where: 'expense_sub_account_id = ?', whereArgs: [subAccountId], orderBy: orderBy);
  }
}
